 ...
 *   3. $DESIGNER_SKILL_CONTEXT_DIR (absolute or cwd-relative) — power-user
...
export function resolveContextDir(cwd = process.cwd()) {
  ...
  const envDir = process.env.DESIGNER_SKILL_CONTEXT_DIR;
 ...
 * the agent never nags. Opt out entirely with DESIGNER_SKILL_NO_UPDATE_CHECK=1.
 */
...
function updateCheckDisabledByConfig(cwd = process.cwd()) {
  ...
  for (const name of ['config.json', 'config.local.json']) {
    try {
      const raw = JSON.parse(fs.readFileSync(path.join(cwd, '.designer-skill', name), 'utf-8'));
...
async function computeUpdateDirective(now = Date.now()) {
  try {
    if (process.env.DESIGNER_SKILL_NO_UPDATE_CHECK) return null;
...
async function cli() {
  ...
  const next = register
    ? `NEXT STEP: This project's register is \`${register}\`. Apply the matching register tone from SKILL.md preflight before producing any design output.`
    : `NEXT STEP: Infer register (brand vs product) from PRODUCT.md and SKILL.md preflight before producing any design output.`;
  parts.push(next);
  if (updateDirective) parts.push(updateDirective);
  process.stdout.write(parts.join('\n\n---\n\n') + '\n');
}
