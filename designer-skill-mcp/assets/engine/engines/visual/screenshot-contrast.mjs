...
async function captureVisualContrastCandidate(page, candidate, viewport) {
  ...
  const token = `designer-skill-contrast-${Date.now()}-${Math.random().toString(36).slice(2)}`;
  const applied = await page.evaluate(({ selector, token, backgroundClipText }) => {
    ...
    if (!el) return false;
    let style = document.getElementById('designer-skill-visual-contrast-hide-style');
    if (!style) {
      style = document.createElement('style');
      style.id = 'designer-skill-visual-contrast-hide-style';
      style.textContent = [
        '[data-designer-skill-visual-contrast-target] {',
        '  color: transparent !important;',
        '  -webkit-text-fill-color: transparent !important;',
        '  text-shadow: none !important;',
        '}',
        '[data-designer-skill-visual-contrast-target][data-designer-skill-bgclip-text="true"] {',
        '  background-image: none !important;',
        '}',
      ].join('\n');
      document.head.appendChild(style);
    }
    el.setAttribute('data-designer-skill-visual-contrast-target', token);
    if (backgroundClipText) el.setAttribute('data-designer-skill-bgclip-text', 'true');
    return true;
  }, {
  ...
  try {
    ...
    await page.evaluate(({ selector }) => {
      try {
        ...
        if (el) {
          el.removeAttribute('data-designer-skill-visual-contrast-target');
          el.removeAttribute('data-designer-skill-bgclip-text');
        }
      } catch {
