...
async function runVisualContrastFallback(page, serializedGroups, options, profile, target) {
  ...
  if (options?.visualContrastBrowser !== false) {
    const browserFindings = await profileFindingsAsync(profile, {
      ...
      browserAnalyses = await page.evaluate(async ({ maxCandidates, scrollOffscreen }) => {
        if (typeof window.designerSkillAnalyzeVisualContrast !== 'function') return [];
        return window.designerSkillAnalyzeVisualContrast({ maxCandidates, scrollOffscreen });
  ...
  if (candidates.length === 0) {
    candidates = await profileStepAsync(profile, {
      ...
      if (typeof window.designerSkillCollectVisualContrastCandidates !== 'function') return [];
      return window.designerSkillCollectVisualContrastCandidates({ maxCandidates });
    }, { maxCandidates }));
  }
...
async function detectUrl(url, options = {}) {
  ...
  try {
    ...
    await profileStepAsync(profile, {
      ...
      window.__DESIGNER_SKILL_CONFIG__ = {
        ...(window.__DESIGNER_SKILL_CONFIG__ || {}),
        autoScan: false,
    ...
    results = await profileFindingsAsync(profile, {
      ...
      serializedGroups = await page.evaluate(() => {
        if (!window.designerSkillDetect) return [];
        return window.designerSkillDetect({ decorate: false, serialize: true });
      });
