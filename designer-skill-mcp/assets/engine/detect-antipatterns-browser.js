 ...
 * Re-scan: window.designerSkillScan()
 */
(function () {
...
const REPEATED_KICKER_SKIP_SELECTOR = [
  ...
  '[class*="breadcrumb" i]',
  '[aria-hidden="true"]',
  '[data-designer-skill-allow-kickers]',
].join(',');
...
if (IS_BROWSER) {
  ...
  const EXTENSION_MODE = (_myScript && _myScript.dataset.designerSkillExtension === 'true')
    || document.documentElement.dataset.designerSkillExtension === 'true';
  ...
  styleEl.textContent = `
    @keyframes designer-skill-reveal {
      from { opacity: 0; }
      to { opacity: 1; }
    }
    .designer-skill-overlay:not(.designer-skill-banner) {
      pointer-events: none;
      outline: 2px solid ${OUTLINE_COLOR};
      border-radius: 4px;
      transition: outline-color 0.15s ease;
      animation: designer-skill-reveal 0.4s cubic-bezier(0.16, 1, 0.3, 1) both;
      animation-play-state: paused;
      border-top-left-radius: 0;
    }
    .designer-skill-overlay.designer-skill-visible {
      animation-play-state: running;
    }
    .designer-skill-overlay.designer-skill-hover {
      outline-color: ${BRAND_COLOR_HOVER};
      z-index: 100001 !important;
    }
    .designer-skill-overlay.designer-skill-hover .designer-skill-label {
      background: ${BRAND_COLOR_HOVER};
    }
    .designer-skill-overlay.designer-skill-spotlight {
      z-index: 100002 !important;
    }
    .designer-skill-overlay.designer-skill-spotlight-dimmed {
      opacity: 0.15 !important;
      animation: none !important;
      filter: blur(3px);
    }
    .designer-skill-spotlight-backdrop {
      position: fixed;
    ...
    .designer-skill-spotlight-backdrop.designer-skill-visible {
      opacity: 1;
    }
    .designer-skill-hidden .designer-skill-overlay${EXTENSION_MODE ? '' : ':not(.designer-skill-banner)'} {
  ...
  function getSpotlightBackdrop() {
    if (!spotlightBackdrop) {
      ...
      spotlightBackdrop.className = 'designer-skill-spotlight-backdrop';
  ...
  function showSpotlight(target) {
    ...
    if (window.__DESIGNER_SKILL_CONFIG__?.spotlightBlur === false) {
    ...
    bd.classList.add('designer-skill-visible');
  }

  function hideSpotlight() {
    spotlightTarget = null;
    if (spotlightBackdrop) spotlightBackdrop.classList.remove('designer-skill-visible');
  }
  ...
  function repositionOverlays() {
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('designer-skill-banner')) continue;
  ...
  const visibilityObserver = new IntersectionObserver((entries) => {
    for (const entry of entries) {
      const overlay = entry.target._designerSkillOverlay;
      if (!overlay) continue;
      if (entry.isIntersecting) {
        ...
        if (!overlay._revealed) {
          ...
          requestAnimationFrame(() => {
            overlay.classList.add('designer-skill-visible');
  ...
  function detachOverlay(overlay) {
    ...
    if (overlay._targetEl && overlay._targetEl._designerSkillOverlay === overlay) {
      visibilityObserver.unobserve(overlay._targetEl);
      delete overlay._targetEl._designerSkillOverlay;
    }
  ...
  document.addEventListener('transitionend', (e) => {
    ...
    for (const o of overlays) {
      if (!o._targetEl || o.classList.contains('designer-skill-banner') || o.style.display === 'none') continue;
  ...
  const highlight = function(el, findings) {
    if (el._designerSkillOverlay) detachOverlay(el._designerSkillOverlay);
    const hasSlop = findings.some(f => RULE_CATEGORY[f.type || f.id] === 'slop');

    const fixed = isInFixedContext(el);
    const rect = el.getBoundingClientRect();
    const outline = document.createElement('div');
    outline.className = 'designer-skill-overlay';
    ...
    label.className = 'designer-skill-label';
    ...
    el._designerSkillOverlay = outline;
    ...
    const onMouseEnter = () => {
      isHovered = true;
      outline.classList.add('designer-skill-hover');
    ...
    const onMouseLeave = () => {
      isHovered = false;
      outline.classList.remove('designer-skill-hover');
  ...
  const showPageBanner = function(findings) {
    ...
    banner.className = 'designer-skill-overlay designer-skill-banner';
    ...
    if (!EXTENSION_MODE) {
      ...
      toggle.addEventListener('click', () => {
        ...
        document.body.classList.toggle('designer-skill-hidden', !overlaysVisible);
  ...
  function buildSelectorSegment(el) {
    ...
    if (el.classList && el.classList.length > 0) {
      const classes = [...el.classList]
        .filter(c => !c.startsWith('designer-skill-') && !isLikelyHashedClass(c))
        .slice(0, 2);
      if (classes.length > 0) {
  ...
  function collectVisualContrastCandidates(options = {}) {
    ...
    for (const el of document.querySelectorAll('*')) {
      ...
      if (el.closest('.designer-skill-overlay, .designer-skill-label, .designer-skill-banner, .designer-skill-tooltip')) continue;
      if (el.closest('[id^="designer-skill-live-"]')) continue;
  ...
  async function sampleVisualBackgroundAtPoint(el, point, textColor, depth = 0) {
    ...
    for (const node of nodes) {
      ...
      if (node.closest?.('.designer-skill-overlay, .designer-skill-label, .designer-skill-banner, .designer-skill-tooltip')) continue;
  ...
  const printSummary = function(allFindings) {
    if (allFindings.length === 0) {
      console.log('%c[designer-skill] No anti-patterns found.', 'color: #22c55e; font-weight: bold');
      return;
    }
    console.group(
      `%c[designer-skill] ${allFindings.length} anti-pattern${allFindings.length === 1 ? '' : 's'} found`,
  ...
  function browserDesignSystemConfig() {
    const raw = window.__DESIGNER_SKILL_CONFIG__?.designSystem;
  ...
  function collectBrowserFindings() {
    ...
    const _disabled = EXTENSION_MODE ? (window.__DESIGNER_SKILL_CONFIG__?.disabledRules || []) : [];
    ...
    for (const el of document.querySelectorAll('*')) {
      // Skip designer-skill's own elements and any descendants (overlays, labels, banner, nav buttons)
      if (el.closest('.designer-skill-overlay, .designer-skill-label, .designer-skill-banner, .designer-skill-tooltip')) continue;
      // Skip browser extension elements (Claude, etc.)
      const elId = el.id || '';
      if (elId.startsWith('claude-') || elId.startsWith('cic-')) continue;
      // Skip the designer-skill live-mode overlay (highlight, tooltip, bar, picker, toast).
    ...
    // Clone the document and strip designer-skill-live overlay nodes before the
    ...
    for (const node of docClone.querySelectorAll('[id^="designer-skill-live-"]')) {
      node.remove();
    }
  ...
  function shouldRunVisualContrast(options = {}) {
    return options.visualContrast === true || window.__DESIGNER_SKILL_CONFIG__?.visualContrast === true;
  }

  function visualContrastOptions(options = {}) {
    const config = window.__DESIGNER_SKILL_CONFIG__ || {};
  ...
  function postExtensionError(err) {
    if (!EXTENSION_MODE) return;
    window.postMessage({
      source: 'designer-skill-error',
      message: err?.message || String(err),
    }, '*');
  }

  function reportVisualContrastError(err, detail = {}) {
    window.dispatchEvent(new CustomEvent('designer-skill-visual-contrast-error', {
      detail: {
        ...detail,
        message: err?.message || String(err),
      },
    ...
    if (EXTENSION_MODE) {
      ...
      console.warn('[designer-skill] visual contrast scan failed', err);
    }
  }
        ...
        waitForVisualPaint()
          ...
          .then(result => {
            ...
            if (added) {
              ...
              window.dispatchEvent(new CustomEvent('designer-skill-visual-contrast-resolved', {
                detail: {
  ...
  if (EXTENSION_MODE) {
    ...
    window.addEventListener('message', (e) => {
      if (e.source !== window || !e.data || e.data.source !== 'designer-skill-command') return;
      if (e.data.action === 'scan') {
        if (e.data.config) window.__DESIGNER_SKILL_CONFIG__ = e.data.config;
        try {
          scan(e.data.config || {});
        } catch (err) {
          postExtensionError(err);
      ...
      if (e.data.action === 'toggle-overlays') {
        const visible = !document.body.classList.contains('designer-skill-hidden');
        document.body.classList.toggle('designer-skill-hidden', visible);
        window.postMessage({ source: 'designer-skill-overlays-toggled', visible: !visible }, '*');
      }
      if (e.data.action === 'remove') {
        clearOverlays();
        styleEl.remove();
        if (spotlightBackdrop) { spotlightBackdrop.remove(); spotlightBackdrop = null; }
        document.body.classList.remove('designer-skill-hidden');
      }
      if (e.data.action === 'highlight') {
        try {
          ...
          if (target) {
            ...
            for (const o of overlays) {
              if (o.classList.contains('designer-skill-banner')) continue;
              const isMatch = o._targetEl === target;
              o.classList.toggle('designer-skill-spotlight', isMatch);
              o.classList.toggle('designer-skill-spotlight-dimmed', !isMatch);
              if (isMatch) {
                // Force the matching overlay visible immediately, don't wait for IntersectionObserver
                o.style.display = '';
                o.style.animation = 'none';
                o.classList.add('designer-skill-visible');
                o._revealed = true;
                positionOverlay(o);
              }
            }
      ...
      if (e.data.action === 'unhighlight') {
        hideSpotlight();
        for (const o of overlays) {
          o.classList.remove('designer-skill-spotlight');
          o.classList.remove('designer-skill-spotlight-dimmed');
        }
      }
    });
    window.postMessage({ source: 'designer-skill-ready' }, '*');
  } else {
    if (window.__DESIGNER_SKILL_CONFIG__?.autoScan !== false) {
      const runAutoScan = () => {
        try {
          scan();
        } catch (err) {
          console.warn('[designer-skill] scan failed', err);
        }
      };
  ...
  window.designerSkillDetect = detect;
  window.designerSkillDetectAsync = detectAsync;
  window.designerSkillScan = scan;
  window.designerSkillScanAsync = scanAsync;
  window.designerSkillCollectVisualContrastCandidates = collectVisualContrastCandidates;
  window.designerSkillAnalyzeVisualContrast = analyzeVisualContrast;
  window.designerSkillGetLastVisualContrastAnalyses = () => lastVisualContrastAnalyses.slice();
}
