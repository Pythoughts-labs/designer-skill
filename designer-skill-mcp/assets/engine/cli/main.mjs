...
} from '../../lib/designer-skill-config.mjs';
import {
  HTML_EXTENSIONS,
  buildImportGraph,
...
function printUsage() {
  console.log(`Usage: designer-skill detect [options] [file-or-dir-or-url...]
...
Options:
  ...
  --no-design-system  Do not load local DESIGN.md / .designer-skill/design.json context
  --help              Show this help message

Project config:
  Respects .designer-skill/config.json and .designer-skill/config.local.json detector
...
Examples:
  designer-skill detect src/
  designer-skill detect index.html
  designer-skill detect https://example.com
  designer-skill detect --json .
  designer-skill detect --no-config src/`);
}
            ...
            if (fwConfig) {
              ...
              if (probe.listening && probe.matched) {
                process.stderr.write(
                  ...
                  `  designer-skill detect http://localhost:${fwConfig.port}\n\n`
                );
