...
Total /32. Below ~20 = significant work; below ~12 = major overhaul. Record findings before editing — the audit is the work plan.
...
6. **Loading / empty / error states** — these are in scope; see `engineering-and-performance.md`.
...
**Self-generate a target when none is provided, if useful:** for a visually-led task with no reference, produce a clean design image first (one readable image per section — don't crop sections out of one compressed board), analyze it, then build to match. Keep heroes calm: short headline (1–3 lines), one focal point, no nested boxes, no filler pills/fake system labels. Image direction details: `aesthetic-systems.md`.
