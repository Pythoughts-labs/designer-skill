...
The bar is a single test: **if someone can look at the interface and say "AI made that" without hesitation, it has failed.** Average is no longer findable. AI landing pages have flooded the internet, so restraint without intent now reads as mediocre, not refined. This file is the cross-register ban-list and the output-completeness contract. Aesthetic-specific palettes, fonts, and shadow systems live in aesthetic-systems.md; the neutral color/typography/layout baseline lives in design-principles.md; motion and reveal tells live in motion-and-interaction.md; loading states and real-data hardening live in engineering-and-performance.md.
...
- **Side-stripe borders:** `border-left`/`border-right` > 1px as a colored accent on cards, list rows, callouts, alerts. Use a full 1px hairline perimeter, a 4-8% surface tint, or a leading glyph instead.
...
### Typography
- **Inter / Roboto / Open Sans / Arial / system default** when personality matters. The most modern move is not the font everyone else uses.
...
### Components & micro-UI
- **Tiny uppercase tracked eyebrow above every section:** the #1 violated rule (appears on 55-95% of generations). Max 1 eyebrow per 3 sections, hero counts as 1; if section A has one, the next 2 cannot. Count `uppercase tracking` instances mechanically before shipping; if count > ceil(sectionCount / 3), it fails. Best alternative: drop it.
...
- [ ] **Color:** no purple/blue gradient, no cream/sand bg, no beige+brass palette, no pure `#000`/`#fff`, no gradient text on headers.
- [ ] **Layout:** no three-equal cards, no nested cards, no decorative hero metric, no side-stripe borders, no reflexive left sidebar.
- [ ] **Type:** not Inter/Roboto/Open Sans by reflex, no reflex display serif, no editorial-typographic lane on a non-editorial brief.
- [ ] **Eyebrows:** `uppercase tracking` count ≤ ceil(sectionCount / 3). No numbered section markers, no split-header, no decoration strips, no scroll cues, no locale strips.
...
- [ ] **Copy:** zero em-dashes (`—`/`–`/` -- `) anywhere visible; no buzzwords; no "Oops!"; verb+object buttons; standalone link text.
...
- [ ] **Slop test:** could a viewer say "AI made that" without doubt? If yes, rework before shipping.
